...
5. Результат каждого прогона коммитится в `runs/<окно>/` и `runs/latest/` (`status.json`, `items.json`, `skeleton.md`, `texts/`, `collect.log`); кэш `raw/` не коммитится. Лог шага «Collect» показывает статусы источников; при блокировках — warning в интерфейсе Actions.
