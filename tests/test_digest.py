...
class LiveGoldenTests(unittest.TestCase):
    """Titles from the first live run (window 08–21.09.2026) and the score the digest needs:
    statistics releases, bank holidays and person-centred announcements out; instruments,
    deadlines and schemes in."""
    CASES = [
        ("Κρατικοί Υπάλληλοι κατά Κατηγορία: Αύγουστος 2026", "απασχόληση", 0),
        ("Απασχόληση στον Ευρύ Δημόσιο Τομέα: 2ο Τρίμηνο 2026", "", 0),
        ("Δείκτης Εργατικού Κόστους: 2ο τρίμηνο 2026", "", 0),
        ("Άδειες Οικοδομής: Μάιος 2026", "", 0),
        ("Εγγραφές Μηχανοκίνητων Οχημάτων: Ιανουάριος-Αύγουστος 2026", "", 0),
        ("Στοιχεία επιτοκίων για καταθέσεις και δάνεια για κάθε πιστωτικό ίδρυμα", "", 0),
        ("Aggregate Cyprus banking sector data (non-performing loans data) with reference date 30 June 2026", "", 0),
        ("Bank holiday", "", 0),
        ("Συγχαρητήρια του Υπουργού Δικαιοσύνης και Δημοσίας Τάξεως για τη σημαντική επιτυχία", "", 0),
        ("Πρόεδρος της Δημοκρατίας: Όλα προχωρούν προς την κατεύθυνση υλοποίησης", "εταιρείες", 0),
        ("O Υπουργός Εργασίας και Κοινωνικών Ασφαλίσεων κ. Μαρίνος Μουσιούττας συμμετείχε στο συνέδριο", "", 0),
        ("Απεργία του Ωρομίσθιου Κυβερνητικού Προσωπικού στις 17 Σεπτεμβρίου 2026", "", 0),
        ("Δήλωση του Υπουργού Οικονομικών κ. Μάκη Κεραυνού αναφορικά με την παράταση της μείωσης του Ειδικού Φόρου Κατανάλωσης", "", 1),
        ("Παρατείνεται η προθεσμία υποβολής Ετήσιας Δήλωσης Παρακράτησης Φόρου", "", 2),
        ("Ο περί Φόρων Κατανάλωσης (Τροποποιητικός) (Αρ. 3) Νόμος του 2026", "", 2),
        ("Γνωστοποίηση δυνάμει του άρθρου 20 του περί της Ίδρυσης και Λειτουργίας Εμπορικού Δικαστηρίου και Ναυτοδικείου Νόμου", "", 2),
        ("ΕΠΙΒΕΒΑΙΩΣΗ ΣΤΟΙΧΕΙΩΝ ΠΡΑΓΜΑΤΙΚΩΝ ΔΙΚΑΙΟΥΧΩΝ ΣΤΟ ΣΥΣΤΗΜΑ ΤΟΥ ΜΗΤΡΩΟΥ", "", 2),
        ("Ανακοίνωση της Υπηρεσίας Κοινωνικών Ασφαλίσεων αναφορικά με την παράταση", "", 2),
        ("Η περί Παροχής Επαρκών Εξηγήσεων σε εξ Αποστάσεως Συμβάσεις Χρηματοοικονομικών Υπηρεσιών Οδηγία του 2026", "", 2),
    ]

    def test_live_titles(self):
        from cylaw_digest.relevance import classify
        for title, body, expected in self.CASES:
            with self.subTest(title=title[:60]):
                self.assertEqual(classify(title, body).score, expected)

    def test_margin_notes_between_blank_lines(self):
        from cylaw_digest import acts
        text = ("(2) Οποιοσδήποτε άλλος όρος στην παρούσα Γνωστοποίηση, έχει την ερμηνεία που του αποδίδεται στο\n"
                "Νόμο.\n\nΗλεκτρονική\nυποβολή\nαίτησης για\nένταξη στη\nΡύθμιση.\n\n"
                "3. Οφειλέτης, ο οποίος επιθυμεί να προβεί σε ρύθμιση της οφειλής του υποβάλλει την αίτηση.\n")
        out = acts.clean_lines(text)
        self.assertNotIn("Ηλεκτρονική", out)
        self.assertIn("στο\nΝόμο.", out)          # the wrapped last word of the paragraph survives

    def test_excerpt_of_article_less_notification(self):
        from cylaw_digest import acts
        text = ("2160\nΕ.Ε. Παρ. ΙΙΙ(Ι)\nΑρ. 6047, 18.9.2026\n\nΑριθμός 348\n"
                "Ο ΠΕΡΙ ΤΗΣ ΙΔΡΥΣΗΣ ΚΑΙ ΛΕΙΤΟΥΡΓΙΑΣ ΕΜΠΟΡΙΚΟΥ ΔΙΚΑΣΤΗΡΙΟΥ ΚΑΙ ΝΑΥΤΟΔΙΚΕΙΟΥ ΝΟΜΟΣ ΤΟΥ 2022\n"
                "(Ν. 69(I)/2022)\nΓνωστοποίηση δυνάμει του άρθρου 20\n"
                "Ο Υπουργός Δικαιοσύνης και Δημοσίας Τάξεως, ασκώντας τις εξουσίες που του παρέχονται δυνάμει των διατάξεων\n"
                "του άρθρου 20 του Νόμου, καθορίζει το κτίριο στην οδό Χαράλαμπου Μούσκου αρ. 4, Λευκωσία, ως τόπο συνεδριάσεων.\n"
                "Έγινε στις 18 Σεπτεμβρίου 2026.\n")
        p = acts.parse_kdp_section("348/2026", text)
        self.assertTrue(p.excerpt.startswith("Ο Υπουργός Δικαιοσύνης"), p.excerpt[:80])
        self.assertNotIn("Έγινε στις", p.excerpt)
